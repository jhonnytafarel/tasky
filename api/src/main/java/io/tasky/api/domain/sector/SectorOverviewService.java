...
public class SectorOverviewService {
    ...
    public SectorOverviewResponse getOverview(SecurityUser user) {
        ...
        List<OrganizationMembership> members = resolveMembers(current, organizationId, validDepartmentIds);
        ...
        boolean restrictToMembers = current.getRole() == Role.employee;
        ...
        return new SectorOverviewResponse(
                ...
                members.stream().map(member -> {
                    ...
                    return new SectorOverviewResponse.MemberSummary(
                            ...
                            load == null ? 0 : load.getOpenActivities(),
    ...
    private Scope resolveScope(OrganizationMembership membership, UUID organizationId) {
        ...
        if (membership.getRole() == Role.manager) {
            return new Scope(new HashSet<>(managerDepartmentRepository.findDepartmentIdsByMembershipId(membership.getId())));
        }
    ...
    private List<OrganizationMembership> resolveMembers(
            OrganizationMembership current, UUID organizationId, Set<UUID> departmentIds) {
    ...
    private record Scope(Set<UUID> departmentIds) {}
}
